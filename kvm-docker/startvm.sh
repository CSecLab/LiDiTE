...
: ${IMAGE:="/image/image"}
: ${CLONE:="/clone"}
: ${KVM_BLK_OPTS:="-drive if=virtio,file="}
# -serial stdio \
...
#    #echo "info status" | socat -t 5 - unix-connect:/qemu-monitor
#    echo "system_powerdown" | socat -t 5 - unix-connect:/qemu-monitor
#    #echo "savevm $TAG" | socat -t 600 - unix-connect:/qemu-monitor
#    #echo "info snapshots" | socat -t 5 - unix-connect:/qemu-monitor
#    #echo "quit" | socat -t 5 - unix-connect:/qemu-monitor
#    exit
#}
...
getNonConflictingIP () {
    ...
    # CONT_IPS
    if ! [[ -z $CONT_IPS ]]; then
       IFS=','; for i in $CONT_IPS
       do
        int=$(echo $i | cut -d: -f 1)
        if [[ "$int" == "$IFACE" ]]; then
         newIP=$(echo $i | cut -d: -f 2)
         newCIDR=$CIDR
        fi
       done
    fi
    ...
    #if [[ "$1" == "10.1.1.150" ]]; then
    #    echo "10.1.1.200 24"
    #else
    #    echo "192.168.250.101 24"
    #fi
...
setupBridge () {
  ...
  if ! [[ -z $IP ]]; then
    newIP=($(getNonConflictingIP $IP $CIDR $iface $CONT_IPS))
...
configureNetworks () {
  ...
  for iface in "${local_ifaces[@]}"; do
    ...
    KVM_NET_OPTS=" -device virtio-net-pci,netdev=net$i,mac=$MAC,bus=pci.0,addr=0x$pci $KVM_NET_OPTS"
    let i++
    let pci++
...
case "$USE_CLONE" in
  [Yy1]* ) USE_CLONE=1;;
  [Nn0]* ) USE_CLONE=0;;
  *      ) log "ERROR" "USE_CLONE incorrect or undefined. It must be one of [Yy1Nn0]"; exit 1;;
esac

if [[ "$USE_CLONE" -eq 1 ]]; then
  KVM_BLK_OPTS="${KVM_BLK_OPTS}${CLONE}"
  # check if linked clone exists
  if [ ! -f $CLONE ]; then
    log "INFO" "Creating linked clone ($CLONE)..."
    qemu-img create -f qcow2 -b $IMAGE $CLONE
  fi
else
  KVM_BLK_OPTS="${KVM_BLK_OPTS}${IMAGE}"
fi

if [[ "$DISABLE_VGA" -eq 0 ]]; then
  : ${KVM_VIDEO_OPTS:="-vga virtio -display none"}
else
